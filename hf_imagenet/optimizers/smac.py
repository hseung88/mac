    ...
    def _capture_activation(
        ...
        if 'attn.qkv' in self.layer_map[module]['name']:
            ...
            if 'exp_avg_q' not in state:
                ...
                state['exp_avg_attn'] = torch.zeros_like(avg_attn, device=avg_attn.device)
            ...
            state['exp_avg_attn'].mul_(stat_decay).add_(avg_attn, alpha=1 - stat_decay)
            ...
            if isinstance(layer, (nn.Linear, nn.Conv2d, nn.LayerNorm)) and layer.weight.grad is not None:
                ...
                if 'attn.qkv' in self.layer_map[layer]['name']:
                    ...
                    if b_updated:
                        ...
                        exp_avg_q = state['exp_avg_q'].div(bias_correction).to(
                            grad_mat.dtype)  # [num_heads, head_dim]
                        exp_avg_k = state['exp_avg_k'].div(bias_correction).to(
                            grad_mat.dtype)  # [num_heads, head_dim]
                        q_inv_list = []
                        k_inv_list = []
