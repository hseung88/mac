...
import numpy as np
import torch
import utils.eigen_utils as utils
from common.logging import get_logger

from .operator import LambdaOperator, Operator
...
def deflated_power_iteration(
    ...
    log.info("beginning deflated power iteration")
    for i in range(num_eigenthings):
        log.info("computing eigenvalue/vector %d of %d" % (i + 1, num_eigenthings))
        ...
        log.info("eigenvalue %d: %.4f" % (i + 1, eigenval))
