...
class CIFAR_EfficientNet(EfficientNet):
    def __init__(self, inverted_residual_setting, dropout, stochastic_depth_prob=0.2, num_classes=10, **kwargs):
        super(CIFAR_EfficientNet, self).__init__(inverted_residual_setting, dropout, stochastic_depth_prob, num_classes, **kwargs)
        
        # Adjust the first convolutional layer to work with CIFAR input size (32x32)
        self.features[0] = nn.Conv2d(3, self.features[0].out_channels, kernel_size=3, stride=1, padding=1, bias=False)

        # Adjust the output classifier to match the number of classes
        lastconv_output_channels = self.classifier[1].in_features
        self.classifier = nn.Sequential(
            nn.Dropout(p=dropout, inplace=True),
            nn.Linear(lastconv_output_channels, num_classes),
        )
...
def efficientnet_cifar(
    *, weights: Optional[WeightsEnum] = None, progress: bool = True, **kwargs: Any
) -> CIFAR_EfficientNet:
    """
    Constructs an EfficientNet architecture adapted for CIFAR datasets.

    Args:
        weights (:class:`~torchvision.models.WeightsEnum`, optional): The
            pretrained weights to use. By default, no pre-trained
            weights are used.
        progress (bool, optional): If True, displays a progress bar of the
            download to stderr. Default is True.
        **kwargs: parameters passed to the ``CIFAR_EfficientNet`` class.
    """
    if weights is not None:
        raise ValueError("Pretrained weights are not supported for CIFAR datasets.")

    inverted_residual_setting, last_channel = _efficientnet_conf("efficientnet_b3", width_mult=1.2, depth_mult=1.4)
    return CIFAR_EfficientNet(
        inverted_residual_setting,
        kwargs.pop("dropout", 0.2),
