...
from .utils.opt_utils2 import extract_patches
...
class ComputeMatGrad:
    ...
    def conv2d(input, grad_output, layer):
        ...
        """
        with torch.no_grad():
            depthwise = layer.groups == input.size(1)
            input = extract_patches(input, layer.kernel_size, layer.stride, layer.padding, depthwise)
...
class ComputeCovA:
    ...
    def conv2d(a, layer):
        #batch_size = a.size(0)
        depthwise = layer.groups == a.size(1)
        a = extract_patches(a, layer.kernel_size, layer.stride, layer.padding, depthwise)
