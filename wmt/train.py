...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Finetune a transformers model on a text translation task with custom optimizers and training from scratch options"
    )
    parser.add_argument(
        "--dataset_name",
        type=str,
    ...
    parser.add_argument(
        "--model_name_or_path",
        type=str,
        help="Path to pretrained model or model identifier from huggingface.co/models. Ignored if training from scratch.",
        required=False,
    )
    parser.add_argument(
        "--config_name",
    ...
    # New argument to choose the optimizer type.
    parser.add_argument(
        "--optimizer_type",
        type=str,
        default="adamw",
    ...
    parser.add_argument(
        "--train_from_scratch",
        action="store_true",
        help="If set, the model will be trained from scratch instead of using pretrained weights.",
    )
...
def main():
    ...
    # Handle repository creation if pushing to hub.
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        if args.with_tracking:
            total_loss = 0
        if args.resume_from_checkpoint and epoch == starting_epoch and 'resume_step' in locals() and resume_step is not None:
