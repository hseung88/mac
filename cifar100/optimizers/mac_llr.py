...
from .utils.mac_utils import (build_layer_map, extract_patches, momentum_step,
                              reshape_grad, trainable_modules)
    ...
    def step(self, closure=None):
        ...
        for layer in self.layer_map:
            if isinstance(layer, (nn.Linear, nn.Conv2d)) and layer.weight.grad is not None:
                ...
                if layer.weight.ndim >= 2 and 'exp_avg' in state:
                    a = state['exp_avg']
                    a_dot_d = torch.square((v * a[None, :]).sum(1)).sum()
                    denom = damping * (grad_mat ** 2).sum() + a_dot_d
                    llr = (v * grad_mat).sum() / (denom * self.alr_factor)
                    self.state[layer.weight]['llr'] = llr.item()
